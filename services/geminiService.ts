...
import { ScoredStock, TradeHistory, AIStockReport, StockCategory, MarketHealth, AIMarketStrategySuggestion, DebateReport, FinalDecision, DebatePoint, NewsArticle, NewsSentimentReport, StrategySettings, AIStrategyAnalysis, PartialStockData, StockData, AnalysisPanel } from '../types';
...
export const getAITopStocks = async (allStaticData: PartialStockData[], strategy: string): Promise<string[]> => {
    if (!config.geminiApiKey) throw new Error("缺少 Gemini API 金鑰。");
    const ai = new GoogleGenAI({ apiKey: config.geminiApiKey });

    // 先根據成交值做一個簡單的預過濾，避免傳送過多數據給 AI
    const preFilteredData = allStaticData
        .filter(s => s.tradeValue && s.tradeValue > 10_000_000) // 僅考慮成交值 > 1000萬
        .sort((a, b) => (b.tradeValue ?? 0) - (a.tradeValue ?? 0))
        .slice(0, 300); // 取前 300 名

    // 傳送給 AI 的數據應盡可能精簡，同時包含策略所需的核心資訊
    const dataSummary = preFilteredData.map(s => ({
        id: s.id,
        n: s.name, // name
        pe: s.peRatio,
        pb: s.pbRatio,
        y: s.dividendYield, // yield
        v: s.tradeValue, // trade value
        // 為了讓 AI 能判斷成長性，我們可以傳入一個簡化的成長指標
        // 注意：在實際應用中，這些數據應該由後端提前準備好
        rg: s.revenueGrowth, // 營收成長率
        roe: s.roe,
        eps: s.eps,
    }));

    const systemInstruction = `你是一位頂尖的量化金融分析師，專長是從大量的靜態市場數據中，根據特定的投資策略，快速篩選出最具潛力的股票。你的任務是接收一個 JSON 格式的股票數據摘要，並根據指定的策略，回傳一個包含前 20 名最符合條件的股票代號的 JSON 陣列。你必須只回傳 JSON 陣列，不包含任何其他文字。`;
    
    const prompt = `
      **投資策略:** ${strategy}

      **策略說明:**
      - **BREAKOUT (波段突破):** 尋找近期價量俱增，可能形成突破趨勢的股票。關注成交值(v)高的個股。
      - **LONG_TERM (長期投資):** 尋找財務穩健的公司。優先考慮高 ROE (roe), 穩定 EPS (eps), 高殖利率 (y)。
      - **DAY_TRADE (當沖標的):** 尋找市場成交值(v)極高、股價波動可能較大的熱門股。
      - **VALUE (價值低估):** 尋找本益比(pe)、股價淨值比(pb)偏低，且殖利率(y)高的潛在便宜股。
      - **GROWTH (成長動能):** 尋找營收年增率(rg)和 EPS (eps) 強勁成長的公司，即使 PE 較高也可接受。

      **市場數據摘要 (JSON):**
      ${JSON.stringify(dataSummary)}

      **任務:**
      請根據上述「${strategy}」策略，從提供的市場數據中篩選出前 20 名最符合條件的股票，並以 JSON 陣列格式回傳它們的 'id'。
...
${finmindApiReference}`;
    ...
    const prompt = `
      ...
      **分析指令:**
      ...
      2.  **Fundamentals**: 評估其營收成長、獲利能力(ROE, EPS)等基本面優勢。
...
export const getGeminiFinalDecision = async (analysisPanels: AnalysisPanel[], stock: StockData, newsReport?: NewsSentimentReport): Promise<FinalDecision> => {
    if (!config.geminiApiKey) throw new Error("缺少 Gemini API 金鑰。");
    const ai = new GoogleGenAI({ apiKey: config.geminiApiKey });
    ...
    const analystReportsContext = analysisPanels.map(panel => `
      ---
      **分析師: ${panel.analystName} (${panel.report.overallStance})**
      - 基本面: ${panel.report.fundamentals.score}/100, 理由: ${panel.report.fundamentals.reasoning}
      - 技術面: ${panel.report.technicals.score}/100, 理由: ${panel.report.technicals.reasoning}
      - 動能: ${panel.report.momentum.score}/100, 理由: ${panel.report.momentum.reasoning}
      - 風險評估: ${panel.report.riskAssessment.score}/100, 理由: ${panel.report.riskAssessment.reasoning}
    `).join('\n');


    const systemInstruction = `你是一位資深的金融投資總監 (CIO)。你的任務是審閱來自你旗下一個 AI 專家小組的多份分析報告，並結合獨立的新聞輿情分析，做出最終的、權威的投資決策。你的決策應綜合所有觀點，並以客戶易於理解的方式呈現。你的回覆必須是結構化的 JSON 格式。
...
${finmindApiReference}`;
    ...
    const finalDecisionSchema = {
        ...
        properties: {
            ...
            synthesisReasoning: { type: Type.STRING, description: "約 50-70 字，說明你如何得出最終結論，如何權衡不同分析師的觀點。例如：'儘管 Groq 提出風險，但我更認同 Gemini 的基本面分析，因其數據更具說服力。'或'所有分析師皆看好，強化了多頭訊號，故維持買進評級。'" }
        },
