...
import { PortfolioHolding, TradeHistory, StrategySettings, SystemLog, YFinanceData, KLineData } from '../types';
import { 
    doc, 
    getDoc, 
    setDoc, 
...
export const fetchStockData = async (stockId: string): Promise<YFinanceData | null> => {
    if (!db) return null;
    try {
        const stockDocRef = doc(db, 'stocks', stockId);
        const docSnap = await getDoc(stockDocRef);
        return docSnap.exists() ? docSnap.data() as YFinanceData : null;
    } catch (error) {
        console.error(`Error getting stock data for ${stockId}:`, error);
        return null;
    }
}

export const getKlineHistory = async (stockId: string): Promise<KLineData[]> => {
    if (!db) return [];
    try {
        // Assuming k-line history is stored in a subcollection for scalability
        const historyCollectionRef = collection(db, 'stocks', stockId, 'kline_history');
        const q = query(historyCollectionRef, orderBy('time', 'asc'));
        const snapshot = await getDocs(q);
        return snapshot.docs.map(doc => doc.data() as KLineData);
    } catch (error) {
        console.error(`Error getting kline history for ${stockId}:`, error);
        return [];
    }
}
...
export const getSystemLogs = async (logLimit: number = 10): Promise<SystemLog[]> => {
    if (!db) return [];
    try {
        const logsCollectionRef = collection(db, 'system_logs');
        const q = query(logsCollectionRef, orderBy('timestamp', 'desc'), limit(logLimit));
        const snapshot = await getDocs(q);
