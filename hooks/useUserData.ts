...
import { updatePortfolioPrices } from '../services/stockService';
...
import { PortfolioHolding, TradeHistory, StockData, ScoredStock, Alert, AlertType, StrategySettings, PartialStockData } from '../types';
import useLocalStorage from './useLocalStorage';
import { IS_FINMIND_CONFIGURED, IS_GEMINI_CONFIGURED, IS_BREVO_CONFIGURED } from '../services/config';
...
export const useUserData = (
    ...
    useEffect(() => {
        const updatePricesAndAlerts = async () => {
            ...
            if (currentPortfolio.length > 0 && currentAllStocks.length > 0 && IS_FINMIND_CONFIGURED) {
                let portfolioWithNewPrices = await updatePortfolioPrices(currentPortfolio);
                
                const fullStockDataForPortfolio = currentAllStocks.filter(s => s.kline && s.kline.length > 0) as StockData[];
        ...
        if (!isDataLoading) {
            // Run once immediately to get the initial state
            updatePricesAndAlerts();

            // Then set up an interval to check every minute during trading hours
            const intervalId = setInterval(() => {
                if (isTradingHours()) {
                    console.log(`[${new Date().toLocaleTimeString()}] In trading hours, updating portfolio...`);
                    updatePricesAndAlerts();
                } else {
                     console.log(`[${new Date().toLocaleTimeString()}] Outside trading hours, skipping portfolio update.`);
                }
            }, 1 * 60 * 1000); // 1 minute
    ...
    useEffect(() => {
        const analyzeRecentTrades = async () => {
            ...
            for (const trade of tradesToAnalyze) {
                ...
                try {
                  const analysis = await getSellTimingAnalysis(trade, postSellPrices);
    ...
    }, [isDataLoading, handleUpdatePostSellAnalysis]);

    const handleAddToPortfolio = useCallback(async (options: AddToPortfolioOptions) => {
        const { stock, shares, entryPrice, screenerData } = options;
    ...
    const handleSell = useCallback(async (holding: PortfolioHolding, sellPrice: number, sharesToSell: number) => {
        const profit = (sellPrice - holding.entryPrice) * sharesToSell;
