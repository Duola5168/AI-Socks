...
import { PortfolioHolding, TradeHistory, StockData, ScoredStock, Alert, AlertType, LayerScores, StrategySettings } from '../types';
import useLocalStorage from './useLocalStorage';
import { config, IS_FINMIND_CONFIGURED, IS_GEMINI_CONFIGURED, IS_BREVO_CONFIGURED } from '../services/config';
...
export const useUserData = (
    ...
    useEffect(() => {
        const updatePricesAndAlerts = async () => {
            ...
            if (currentPortfolio.length > 0 && currentAllStocks.length > 0 && IS_FINMIND_CONFIGURED) {
                ...
                const newAlerts = checkPortfolio(portfolioWithNewPrices, currentAllStocks, settings);
                setAlerts(newAlerts);
        ...
        if (!isDataLoading) {
            ...
            const intervalId = setInterval(updatePricesAndAlerts, 60000);
            return () => clearInterval(intervalId);
        }
    }, [isDataLoading, user, setPortfolio, settings]);
    ...
    const handleAddToPortfolio = useCallback(async (options: AddToPortfolioOptions) => {
        ...
        const newHolding: PortfolioHolding = {
            ...
            entryPrice,
            shares,
            currentPrice: stock.kline.length > 0 ? stock.kline[stock.kline.length - 1].close : entryPrice,
