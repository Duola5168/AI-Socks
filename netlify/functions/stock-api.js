...
const { VITE_FINMIND_API_TOKEN, VITE_NEWS_API_KEY, VITE_GITHUB_API_KEY, VITE_NEWS2_API_KEY } = process.env;
...
exports.handler = async function (event) {
  ...
  try {
    ...
    switch (source) {
      case 'twse': {
        const { endpoint, category, ...restOfTwseParams } = queryParams;
        if (!endpoint) {
          throw new Error('TWSE endpoint is required');
        }
        if (!category) {
            throw new Error('TWSE category is required. e.g., "company" or "exchangeReport"');
        }
        const twseParams = new URLSearchParams(restOfTwseParams);
        targetUrl = `${TWSE_BASE_URL}/${category}/${endpoint}?${twseParams.toString()}`;
        break;
      }
      ...
      case 'newsapi2': {
        if (!VITE_NEWS2_API_KEY) {
            throw new Error("News2 API key is not configured on the server.");
        }
        delete queryParams.source;
        const news2Params = new URLSearchParams(queryParams);
        targetUrl = `https://api.webz.io/filterWebContent?token=${VITE_NEWS2_API_KEY}&${news2Params.toString()}`;
        break;
      }
