...
import { TradeHistory, StrategySettings } from '../types';
...
export const History: React.FC<HistoryProps> = ({ tradeHistory, onUpdateAnalysis, isGeminiConfigured, settings }) => {
  ...
  const handleAnalysis = async (tradeId: string) => {
    ...
    if (trade) {
      ...
      const analysisResult = await getPostTradeAnalysis(settings.analystPanel.geminiModel, trade);
