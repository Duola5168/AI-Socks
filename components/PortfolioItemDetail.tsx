...
import { PortfolioHolding, StockData, Alert, AlertType, PartialStockData } from '../types';
...
const StockChart: React.FC<{ data: StockData['kline'] }> = ({ data }) => {
    ...
    useEffect(() => {
        ...
        candleSeries.setData(data.map(d => ({...d, time: d.time.replace(/-/g, '/')})));
        ...
        const calculateMA = (period: number) => {
            ...
            for (let i = period - 1; i < data.length; i++) {
                ...
                result.push({ time: data[i].time.replace(/-/g, '/'), value: sum / period });
            }
            return result;
        };
        
        const ma5 = chart.addLineSeries({ color: 'cyan', lineWidth: 2 });
        ma5.setData(calculateMA(5));

        const ma20 = chart.addLineSeries({ color: 'yellow', lineWidth: 2 });
  ...
  return (
    <div className="bg-gray-800 border border-t-0 border-gray-700 rounded-b-lg p-4 sm:p-6 space-y-6">
      ...
      <div>
          ...
          {stockData && stockData.kline ? <StockChart data={stockData.kline} /> : <p className="text-gray-500 text-center py-10">無詳細 K 線資料可供顯示。</p>}
      </div>
    </div>
  );
};