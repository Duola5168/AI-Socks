...
import { ScoredStock, StockCategory, CollaborativeAIReport, LayerScores, DebateReport, DebatePoint, FinalDecision, NewsSentimentReport } from '../types';
import { 
...
const DebatePanel: React.FC<{ pro: DebateReport, con: DebateReport }> = ({ pro, con }) => {
    ...
    const DebateCard: React.FC<{ side: 'pro' | 'con', title: string, report: DebateReport }> = ({ side, title, report }) => (
        <div className={`p-3 rounded-lg ${side === 'pro' ? 'bg-green-900/40' : 'bg-red-900/40'}`}>
            <h5 className={`font-semibold mb-3 flex items-center gap-2 ${side === 'pro' ? 'text-green-300' : 'text-red-300'}`}>
                {side === 'pro' ? <BrainCircuitIcon className="w-4 h-4"/> : <GroqIcon className="w-4 h-4"/>}
                {title} (立場: {report.overallStance})
            </h5>
            <div className="space-y-3">
                {debatePointsConfig.map(({ key, label }) => {
                    const point = report[key as keyof DebateReport] as DebatePoint;
                    return (
                        <div key={key}>
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <DebateCard side="pro" title="正方分析 (Gemini)" report={pro} />
            <DebateCard side="con" title="反方分析 (Groq)" report={con} />
        </div>
    );
};
...
const CollaborativeReportDisplay: React.FC<{ report: CollaborativeAIReport }> = ({ report }) => {
    ...
    const { finalDecision, proAnalysis, conAnalysis, newsReport } = report;

    return (
        <div className="space-y-4">
            <h4 className="text-sm font-semibold text-gray-300 flex items-center"><ChatBubbleLeftRightIcon className="w-5 h-5 mr-2 text-teal-400" /> AI 三階段評比報告</h4>
...
const CollaborationLoader: React.FC = () => {
    ...
    return (
        <div className="text-center text-teal-400 p-4">
            ...
            <p className="mt-4 text-sm font-semibold transition-opacity duration-500">{stages[stage]}</p>
        </div>
    );
};
...
export const StockDetailView: React.FC<StockDetailViewProps> = ({ 
  ...
  const { stock, score, aiReport, isCollaborating, collaborativeReport, analysisError } = scoredStock;
            ...
            <div className="space-y-6">
                ...
                {aiReport && (
                    <div className="bg-gray-900/50 p-4 rounded-lg">
                        ...
                        {isCollaborating && <CollaborationLoader />}
                        {analysisError && (
