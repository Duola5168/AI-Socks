...
export const PortfolioTracker: React.FC<PortfolioTrackerProps> = ({ holdings, alerts, onSell, onAddToPortfolio, onToggleTrackingMode, isPortfolioFull, allStocks }) => {
  ...
  const handleAddToPortfolioConfirm = async (stock: PartialStockData, shares: number, price: number, position: 'long' | 'short') => {
      const success = await onAddToPortfolio({ stock, shares, entryPrice: price, position });
      if (success) {
