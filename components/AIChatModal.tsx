...
import { ScoredStock, ChatMessage } from '../types';
...
export const AIChatModal: React.FC<AIChatModalProps> = ({ isOpen, onClose, stock: scoredStock }) => {
    const { stock, aiReport } = scoredStock;
    ...
    useEffect(() => {
        if (isOpen && config.geminiApiKey && !chat) {
            ...
            const initialContext = `
                ...
                初步 Gemini 分析報告:
                - 分類: ${aiReport?.category}
                - 原因: ${aiReport?.reasoning}
                - 進場建議: ${aiReport?.entryAnalysis}
                - 支撐/壓力: ${aiReport?.supportLevel} / ${aiReport?.resistanceLevel}
            ...
            setMessages([
                ...
                { role: 'model', content: `你好！我是您的 AI 分析師。關於 ${stock.name}，有什麼想深入了解的嗎？` }
            ]);
            setError(null);
        } else if (isOpen && !config.geminiApiKey) {
            setError("Gemini API Key 未設定，無法使用對話功能。");
        }
    }, [isOpen, stock, aiReport, chat]);
